...
function ReconCheck({ label, ok, count, okText, badText }) {
  return (
    <div className="rounded-xl border border-gray-100 bg-white p-3">
      ...
      <p className={`mt-1 text-xs ${ok ? 'text-green-700' : 'text-amber-700'}`}>
      ...
      <div className="h-full w-full max-w-lg overflow-y-auto bg-white shadow-2xl" onClick={(e) => e.stopPropagation()}>
        ...
        {loading ? (
          ...
          <div className="space-y-6 px-5 py-5">
            {/* Balance */}
            <div className="rounded-xl bg-gradient-to-br from-[#05243F] to-[#0A3B66] p-5 text-white">
              <p className="text-xs text-white/60">Balance</p>
              <p className="mt-1 text-3xl font-semibold">{naira(wallet?.balance_kobo)}</p>
              <div className="mt-3 flex items-center gap-2">
                <StatusBadge tone={frozen ? 'red' : 'green'}>{frozen ? 'Frozen' : 'Active'}</StatusBadge>
                <button onClick={toggleFreeze} disabled={busy}
                  className="ml-auto flex items-center gap-1 rounded-lg bg-white/10 px-3 py-1 text-xs font-medium hover:bg-white/20 disabled:opacity-50">
            ...
            <div className="rounded-xl border border-gray-100 bg-gray-50 p-4">
              <p className="mb-3 text-sm font-semibold text-gray-900">Manual adjustment</p>
              <div className="mb-3 flex gap-2">
                {['credit', 'debit'].map((d) => (
                  <button key={d} onClick={() => setDirection(d)}
                    className={`flex-1 rounded-lg py-2 text-sm font-semibold capitalize transition-all ${direction === d ? (d === 'credit' ? 'bg-green-600 text-white' : 'bg-red-600 text-white') : 'bg-white text-gray-700 border border-gray-200'}`}>
                    {d}
                  </button>
                ))}
              </div>
                  ...
                  {(data.entries || []).map((e) => {
                    ...
                    return (
                      <li key={e.id} className="flex items-center gap-3 px-3 py-2.5">
                        <span className={`flex h-8 w-8 items-center justify-center rounded-full ${credit ? 'bg-green-50 text-green-600' : 'bg-red-50 text-red-600'}`}>
                          {credit ? <ArrowDownLeftIcon className="h-4 w-4" /> : <ArrowUpRightIcon className="h-4 w-4" />}
                        </span>
                        <div className="min-w-0 flex-1">
                          <p className="text-sm font-medium text-gray-900">{REASON_LABEL[e.reason] || e.reason}</p>
                        ...
                        <div className="text-right">
                          <p className={`text-sm font-semibold ${credit ? 'text-green-600' : 'text-gray-900'}`}>{credit ? '+' : '−'}{naira(e.amount_kobo)}</p>
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        icon={WalletIcon}
        title="Wallets"
        subtitle="Manage user wallet balances, adjustments and freezes."
      />
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <StatCard
          ...
          value={stats ? naira(stats.total_liability_kobo) : '—'}
          hint="owed to users"
          color="blue"
        />
        <StatCard
          icon={WalletIcon}
          label="Wallets"
          value={stats ? stats.wallet_count : '—'}
          color="gray"
        />
      </div>
      ...
      {/* Reconciliation */}
      {recon && (
        <div className={`rounded-xl border p-5 shadow-sm ${allReconOk(recon) ? 'border-green-100 bg-green-50/40' : 'border-amber-200 bg-amber-50'}`}>
          <div className="mb-3 flex items-center justify-between">
            ...
            <button onClick={loadRecon} disabled={reconLoading} className={BTN_SECONDARY}>
      ...
      {/* Search */}
      <form onSubmit={(e) => { e.preventDefault(); setSearch(searchInput); }} className="flex max-w-md gap-2">
      ...
      <div className={`overflow-hidden ${CARD}`}>
        {loading ? (
          ...
          <EmptyState
            ...
            body={search ? `No wallets match “${search}”. Try a different name or email.` : 'Wallets appear here once users fund their accounts.'}
          />
        ) : (
              ...
              <tbody className="divide-y divide-gray-100">
                {wallets.map((w) => (
                  <tr key={w.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-sm font-semibold text-gray-900">{naira(w.balance_kobo)}</td>
                    <td className="px-4 py-3">
